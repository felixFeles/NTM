...
6. **Generate:** run `scripts/build_scene_references.py` on the resolved scene before creating requests; it selects validated, timeline/state-applicable assets. Create one provider-neutral panel request per JSON file at `generation/requests/<chapter_id>/<request_id>.json`; the filename must exactly match `request_id`. Validate paths and visual-reference gates with `scripts/validate_generation_requests.py`. Each request must conform to `panel-generation-request.schema.json` and contain `timeline_order`, prompt, negative constraints, composition, camera, lighting, style, text/bubbles, resolved entity state, spatial relations, references, textures, provider parameters, and reproducibility hashes. A provider adapter may then produce assets.
...
For every visually important entity, retain `references/<entity_id>/manifest.json` and its image files. The mandatory manifest defines a canonical identity sheet, required views, state variants, palette, materials, texture maps, geometry constraints, and version history. Every `identity_sheet`, `outfit_sheet`, `material_texture`, `surface_detail`, `energy_effect`, `location_sheet`, or `damage_variant` asset must have a SHA-256 hash, validation status, provenance, and timeline/state/role query rules. A generation request is blocked if a visible entity has no applicable validated canonical identity sheet, or a requested texture lacks an applicable validated `material_texture` map.
